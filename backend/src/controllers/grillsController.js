...
export async function getGrillById(req, res) {
  try {
    const grill = await Grill.findById(req.params.id).populate(
      "creator",
      "username name email"
    );
...
export async function updateGrill(req, res) {
  try {
    ...
    if (grill.creator.toString() !== req.userId) {
      return res
        .status(403)
        .json({ error: "Not authorized to update this grill" });
    }
...
export async function deleteGrill(req, res) {
  try {
    ...
    if (grill.creator.toString() !== req.userId) {
      return res
        .status(403)
        .json({ error: "Not authorized to delete this grill" });
    }
    ...
    res
      .status(200)
      .json({ message: "Grill deleted successfully", deletedGrill: grill });
  } catch (error) {
    res.status(500).json({ error: error.message });
  }
}
...
export async function getLeaderboard(req, res) {
  try {
    const topGrills = await Grill.aggregate([
      {
        $addFields: {
